...
def task_sync_daily_alarm(bot):
    ...
    try:
        ...
        if already_notified:
            ...
            if isinstance(raw_dt, dict): raw_dt = raw_dt.get('date') or raw_dt.get('dateTime')
            event_datetime = raw_dt if isinstance(raw_dt, datetime) else datetime.combine(raw_dt, datetime.min.time())
            
            resume_time = event_datetime + timedelta(minutes=30)
            if resume_time < datetime.now():
                resume_time = datetime.now() + timedelta(hours=1)
                
            logger.info(f"'{summary}' déjà notifié (Hash match). Pause jusqu'à {resume_time}")
        ...
        if event_datetime.date() != datetime.now().date():
            logger.info(f"'{summary}' n'est pas pour aujourd'hui.")
            bot.scheduler.add_job(task_sync_daily_alarm, 'date', run_date=datetime.now() + timedelta(hours=1), args=[bot])
            return
        ...
        if alarm_time <= datetime.now():
            ...
            logger.info(f"Alarme programmée à {alarm_time}")
            bot.loop.call_soon_threadsafe(
                lambda: bot.scheduler.add_job(
                    lambda: fire_alarm(bot, event),
                    trigger="date", run_date=alarm_time, id="dynamic_agenda_alarm", replace_existing=True
                )
            )
...
def fire_alarm(bot, event):
    ...
    logger.debug("Executing fire_alarm")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            event_hash = hashlib.md5(summary.encode('utf-8')).hexdigest()
...
def _execute_save(bot, m):
    ...
    if 'concert' in m.content.lower():
        ics_path = path.parent / f"{Path(filename).stem}.ics"
        filename = None
        if m.attachments:
    ...
    ics_path = path.parent / f"{Path(filename).stem}.ics"
    try:
        logger.info(f"Tentative de création ICS : {ics_path}")
        generate_ics_from_template(ics_path, event, date, venue)
        
        if not ics_path.exists():
            logger.error(f"ÉCHEC : Le fichier {ics_path} n'existe pas après appel de generate_ics_from_template")
            files_to_send = []
        else:
            logger.info(f"SUCCÈS : Fichier ICS créé.")
            files_to_send = [str(ics_path)]

    except Exception as e:
        logger.error(f"Erreur lors de la génération ICS : {e}")
        files_to_send = []

    logger.info(f"SUCCÈS : Fichier ICS créé ????. {files_to_send}")
    ...
    return f"Ticket sauvé pour **{event}**"


def _format_week(label, events):
    if not events: return f"No events for {label}."
